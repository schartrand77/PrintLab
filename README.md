...
- PrintLab Slicer:
  `ORCASLICER_BINARY` points to the OrcaSlicer executable used by `/slicer` such as `orca-slicer.exe` on Windows or `orca-slicer` on Unix-like installs. If unset, PrintLab checks PATH and common install locations. `ORCA_SLICER_BINARY` is supported as a legacy alias.
